...
class Login {
    ...
    public function loginProcess(){
        ...
        if($row = mysql_fetch_array($query)){
            ...
            if($total > 0){
                $ip = get_client_ip();
                $table_field = array('logininfo_empl_id','logininfo_ip','logininfo_type');
                $table_value = array($row['empl_id'],$ip,'Backend');
                $remark = system_datetime . " : Insert Employee login record ";
                $this->save->SaveData($table_field,$table_value,'db_logininfo','logininfo_id',$remark);
            ...
            {
                    $sql2 = "SELECT *, COUNT(*) as total FROM db_applicant WHERE applicant_login_email = '$this->login_email' AND applicant_login_password = '$this->login_password'";
                    ...
                    if($row2 = mysql_fetch_array($query2)){
                        ...
                        if($total > 0){
                            $ip = get_client_ip();
                            $table_field = array('logininfo_empl_id','logininfo_ip','logininfo_type');
                            $table_value = array($row2['applicant_id'],$ip,'Backend');
                            $remark = system_datetime . " : Candidate login record ";
                            $this->save->SaveData($table_field,$table_value,'db_logininfo','logininfo_id',$remark);
                            $_SESSION['empl_id'] = $row2['applicant_id'];
                            $_SESSION['empl_name'] = $row2['applicant_name'];
                            $_SESSION['empl_email'] = $row2['applicant_email'];
                            $_SESSION['empl_code'] = "";
                            $_SESSION["empl_login_expiry"] = $session_expiry_time;
                            $_SESSION['empl_group'] = '5';
                            $_SESSION['empl_department'] = "";
                            $_SESSION['empl_outlet'] = "";
                            $_SESSION['empl_outlet_code'] = "";
                            $_SESSION['empl_login_email'] = $row2['applicant_email'];                        
                        }
                    }
            }
        }else{    
